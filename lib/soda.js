...
module.exports = function(gulp, h_config={}) {
	...
	**/
	...
	// prep all task options hash
	let h_all_task_options ={};
	...
	for(let s_dir in h_domain) {
		...
		a_recipe_list.forEach((s_recipe_target, i_recipe) => {
			...
			gulp.task(s_task, a_task_deps, (f_done_task) => {
				...
				// make options function for this task
				let f_task_options = (s_other) => {
					return h_all_task_options[s_other];
				};

				// make options hash for this task, save to persistent options hash
				h_all_task_options[s_task] = Object.assign(f_task_options,
					h_options['*'] || {},
					h_options[s_recipe] || {},
					h_options[s_task] || {});
